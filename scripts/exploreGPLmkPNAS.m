...
for k=1:GroupSize
    [~,locGPLmkIdx(k,1)] = max(ptuq(NamesDelimit(k,1):NamesDelimit(k,2)));
    GPLmkIdx(k,1) = locGPLmkIdx(k,1)+NamesDelimit(k,1)-1;
%     [~,GPLmkIdx(k,1)] = max(ptuq(NamesDelimit(k,1):NamesDelimit(k,2)));
%     locGPLmkIdx(k,1) = GPLmkIdx(k,1)-NamesDelimit(k,1)+1;
end
...
%     hold on
%     scatter3(meshList{j}.V(1,locGPLmkIdx(j,1)),...
%         meshList{j}.V(2,locGPLmkIdx(j,1)),...
%         meshList{j}.V(3,locGPLmkIdx(j,1)),...
%         20,'g','filled');
% end
...
for k=2:numLmk
    ...
    for j=1:GroupSize
        [~,locGPLmkIdx(j,k)] = max(ptuq(NamesDelimit(j,1):NamesDelimit(j,2)));
        GPLmkIdx(j,k) = locGPLmkIdx(j,k)+NamesDelimit(j,1)-1;
    end
