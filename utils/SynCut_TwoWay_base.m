...
while true
    ...
    [RelaxSolPerEdgeFrustVec, RelaxSolPerEdgeFrustMat] =...
                          getPerEdgeFrustration(G, GCL_W, d, RelaxSolCell);
    if iterCounter == 1
...
%         axis square
    ...
    while true
        clusterLabel =...
            ...
            numKmeans,bandwidth,adjType,debugFlag,G.adjMat);
...
%     end
    ...
    [combinedSolPerEdgeFrustVec, combinedSolPerEdgeFrustMat] =...
            getPerEdgeFrustration(G, GCL_W, d, combinedSolCell);
%     if debugFlag        
...
%         ccCorr = ccCorr+RelaxSolPerEdgeFrustMat(ccRowIdx(j),ccColIdx(j))*CollageSolCell{ccColIdx(j)}'*GCL_W(blockRowIdx, blockColIdx)'*CollageSolCell{ccRowIdx(j)};
    ...
    [CollageSolPerEdgeFrustVec, CollageSolPerEdgeFrustMat] =...
                       getPerEdgeFrustration(G, GCL_W, d, CollageSolCell);
    if debugFlag
    ...
    wAdjMat = sparse(wRowIdx,wColIdx,exp(-wVals),size(wAdjMat,1),size(wAdjMat,2));
