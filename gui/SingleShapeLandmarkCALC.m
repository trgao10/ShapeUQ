function varargout = SingleShapeLandmarkCALC(varargin)
% SINGLESHAPELANDMARKCALC MATLAB code for SingleShapeLandmarkCALC.fig
%      SINGLESHAPELANDMARKCALC, by itself, creates a new SINGLESHAPELANDMARKCALC or raises the existing
%      singleton*.
%
%      H = SINGLESHAPELANDMARKCALC returns the handle to a new SINGLESHAPELANDMARKCALC or the handle to
%      the existing singleton*.
%
%      SINGLESHAPELANDMARKCALC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SINGLESHAPELANDMARKCALC.M with the given input arguments.
%
%      SINGLESHAPELANDMARKCALC('Property','Value',...) creates a new SINGLESHAPELANDMARKCALC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SingleShapeLandmarkCALC_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SingleShapeLandmarkCALC_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SingleShapeLandmarkCALC
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SingleShapeLandmarkCALC_OpeningFcn, ...
                   'gui_OutputFcn',  @SingleShapeLandmarkCALC_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SingleShapeLandmarkCALC is made visible.
function SingleShapeLandmarkCALC_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SingleShapeLandmarkCALC (see VARARGIN)

% Choose default command line output for SingleShapeLandmarkCALC
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SingleShapeLandmarkCALC wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SingleShapeLandmarkCALC_OutputFcn(hObject, eventdata, handles) 
