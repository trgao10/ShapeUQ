function varargout = SingleShapeLandmarkAST(varargin)
% SINGLESHAPELANDMARKAST MATLAB code for SingleShapeLandmarkAST.fig
%      SINGLESHAPELANDMARKAST, by itself, creates a new SINGLESHAPELANDMARKAST or raises the existing
%      singleton*.
%
%      H = SINGLESHAPELANDMARKAST returns the handle to a new SINGLESHAPELANDMARKAST or the handle to
%      the existing singleton*.
%
%      SINGLESHAPELANDMARKAST('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SINGLESHAPELANDMARKAST.M with the given input arguments.
%
%      SINGLESHAPELANDMARKAST('Property','Value',...) creates a new SINGLESHAPELANDMARKAST or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SingleShapeLandmarkAST_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SingleShapeLandmarkAST_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SingleShapeLandmarkAST
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SingleShapeLandmarkAST_OpeningFcn, ...
                   'gui_OutputFcn',  @SingleShapeLandmarkAST_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SingleShapeLandmarkAST is made visible.
function SingleShapeLandmarkAST_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SingleShapeLandmarkAST (see VARARGIN)

% Choose default command line output for SingleShapeLandmarkAST
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SingleShapeLandmarkAST wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SingleShapeLandmarkAST_OutputFcn(hObject, eventdata, handles) 
...
data_path = '~/Work/MATLAB/DATA/AST_CALC/ast/';
